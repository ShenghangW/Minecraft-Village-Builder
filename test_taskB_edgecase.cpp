...
std::cout << "COMPONENT 4: Furniture Placement" << std::endl;
    ...
    if (std::cin >> originX >> originY >> originZ) {
...
while (std::cin >> originX >> originY >> originZ >> boundX >> boundY >> boundZ >> entranceX >> entranceY >> entranceZ) {
    ...
    // Place furniture in each room
        std::cout << std::endl;
        std::cout << "\tDecorating rooms with furniture..." << std::endl;
        for (Room& room : rooms) {
            std::string theme = room.getTheme();
            std::cout << "\t" << theme << " at ("     << room.getOrigin().x << "," << room.getOrigin().z << ") to (" << room.getBound().x << "," << room.getBound().z << "):" << std::endl;
